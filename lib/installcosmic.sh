...
_header "Copying \"16x16\" icons"
    (
     ...
     cp -a "${_basedir}/wm/cosmic/16x16/status" .
     ln -s status panel
     ln -s apps categories
    )
_footer
...
_header "Copying \"22x22\" icons"
    (
     ...
     cp -a "${_basedir}/wm/cosmic/22x22/status" .
     ln -s status panel
...
_header "Copying \"24x24\" icons"
    (
     ...
     cp -a "${_basedir}/wm/cosmic/24x24/status" .
     cp -a "${_basedir}/wm/cosmic/24x24/animations" .
     ln -s status panel
     ln -s ../16x16/apps apps
     ln -s ../16x16/categories categories
    )
...
_header "Copying \"32x32\" icons"
    (
     ...
     cp -a "${_basedir}/wm/cosmic/32x32/status" .
...
_header "Copying \"scalable\" icons"
    (
     cd scalable || exit 1
     for i in $(find "${_basedir}/wm/cosmic/scalable/" -mindepth 1 -type d)
      do cp -a "${i}" .
    done
...
_header "Copying \"scalable-32\" icons"
     cp -ar "${_basedir}/wm/cosmic/scalable-max-32/" .
_footer
...
cp -a "${_basedir}/wm/cosmic/index.theme" .
