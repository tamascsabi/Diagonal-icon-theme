...
_line
printf ";;; \n"
printf ";;; %bWith this program you can recolor the folders of the%b\n" "${blue}" "${reset}"
printf ";;; %balready installed Diagonal icon theme.%b\n" "${blue}" "${reset}"
printf ";;;\n"
_line
}
            ...
            if [ -d "${path}" ]
               then if [ "${dir}" = "${_sys_dir}" ] && [ "$(id -u)" -ne 0 ]
                       then if [ -n "${_auto_priv}" ]
                               then printf ";;; \n"
                                    printf ";;; %bColorize:%b %b%s%b\n" "${blue}" "${reset}" "${red}" "${path}" "${reset}"
                                    printf ";;; \n"
                                    $_auto_priv env _basekt="${dir}/${base}" _rootdir="${_rootdir}" ./lib/colorize.sh
                               else printf ";;; \n"
                       ...
                       else printf ";;; %bColorize:%b %b%s%b\n" "${blue}" "${reset}" "${red}" "${path}" "${reset}"
                            printf ";;; \n"
                            env _basekt="${dir}/${base}" _rootdir="${_rootdir}" ./lib/colorize.sh
                    fi
            fi
     done
}
...
printf ";;; \n"
printf ";;; %bEnter the number of the icon pack to be colorized:%b \n" "${blue}" "${reset}"
      ...
      do eval "base=\$opt_${i}"
         if [ "${i}" -eq "${choice}" ] 2>/dev/null
            then printf ";;; \n"
                 _line
                 printf ";;; \n"
                 printf ";;; %bYou will colorize the following icon themes:%b \n" "${blue}" "${reset}"
                 printf ";;; \n"
                 printf ";;; %b%s%b\n" "${red}" "${base}" "${reset}"
                 printf ";;; \n"
